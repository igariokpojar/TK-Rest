...
@Service
// creat one service with is created and deleting Users
public class KeycloakServiceImpl implements KeycloakService {
    ...
    public Response userCreate(UserDTO userDTO) {
        ...
        credential.setTemporary(false); // not temporary
        ...
        Keycloak keycloak = getKeycloakInstance();
                                                             //keycloakProperties->keycloak.realm=cydeo-dev / from app properties
 ...
 // this code does-> it is going to keycloak, it checks all the roles there, and then it is taking userDTO.getRole()
        // , and there when ever it is matching with the role in the keycloak it is assign that role in the keycloak under that user.
        ...
        keycloak.close(); // close the keycloak instance
        return result;
    }
    ...
    private Keycloak getKeycloakInstance(){ // create an Instance
        // all this info is come from application properties (Keycloak properties)
